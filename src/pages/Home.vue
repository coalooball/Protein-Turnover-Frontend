<template>
    ...
    <q-expansion-item v-model="expandedClickhouseExpansion" dense dense-toggle expand-separator label="Clickhouse Information">
    ...
    <q-dialog v-model="addClickhouseInformationModel" persistent>
        <q-card style="min-width: 350px">
            ...
            <q-card-section class="q-pt-none">
                ...
                <q-input dense label="Password" stack-label v-model="clickhousepassword" autofocus @keyup.enter="createClickhouseInformation" />
            </q-card-section>
    ...
    <q-dialog v-model="deleteClickhouseInformationModel" persistent>
        <q-card style="min-width: 350px">
            ...
            <q-card-section class="q-pt-none">
                <q-input dense label="ID" stack-label v-model="clickhouseDeleteID" autofocus @keyup.enter="deleteClickhouseInformation" />
            </q-card-section>
...
function createClickhouseInformation() {
    ...
    fetch("/api/create_clickhouse_information", {
        method: "POST",
        headers: {
...
function readAllClickhouseInformation() {
    ...
    fetch("/api/read_all_clickhouse_information")
        .then((response) => response.json())
        .then((data) => {
            console.log(data);
            if (data.success) {
...
function deleteClickhouseInformation() {
    ...
    fetch("/api/delete_clickhouse_information", {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
        },
